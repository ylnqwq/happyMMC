...
人工蜂群（Artificial Bee Colony，ABC）算法是一种受蜜蜂采蜜行为启发的群智能优化算法。算法将候选解看作食物源，并通过三类蜜蜂协同搜索：雇佣蜂在当前食物源附近开发新解，跟随蜂根据食物源质量选择并进一步搜索，侦察蜂在长期没有改进时放弃原食物源并随机探索新的区域。通过“局部开发”和“全局探索”的交替，ABC 能够在较少先验信息的情况下求解复杂的非线性优化问题。

本项目中的 MOIABC（Multi-Objective Improved ABC）面向多目标优化进行了扩展：使用 Pareto 非支配解档案保存多个折中解，并结合优良点初始化、锦标赛选择、精英增强、档案引导和自适应淘汰等机制，提高收敛速度、解集分布性和多目标搜索稳定性。
...
敏感度和消融实验的变量按实验分组，避免长变量名挤压文档版面：

### 精英率-淘汰率敏感度

- 重复次数：`MOIABC_SENSITIVITY_RUN_TIMES`
- 测试集与函数：`MOIABC_SENSITIVITY_SUITES`、`MOIABC_SENSITIVITY_FUNCTION_IDS`
- 输出目录：`MOIABC_ELITE_ELIMINATION_SENSITIVITY_OUTPUT_DIR`

### 档案引导率敏感度

- 重复次数：`MOIABC_ARCHIVE_RATE_SENSITIVITY_RUN_TIMES`
- 测试集与函数：`MOIABC_ARCHIVE_RATE_SENSITIVITY_SUITES`、`MOIABC_ARCHIVE_RATE_SENSITIVITY_FUNCTION_IDS`
- 输出目录：`MOIABC_ARCHIVE_RATE_SENSITIVITY_OUTPUT_DIR`
- 扫描参数：`MOIABC_ARCHIVE_RATE_SENSITIVITY_ARCHIVE_RATES`

### 消融实验

- 重复次数：`MOIABC_ABLATION_RUN_TIMES`
- 测试集与函数：`MOIABC_ABLATION_SUITES`、`MOIABC_ABLATION_FUNCTION_IDS`
- 输出目录：`MOIABC_ABLATION_OUTPUT_DIR`
- 启用变体：`MOIABC_ABLATION_VARIANTS`
