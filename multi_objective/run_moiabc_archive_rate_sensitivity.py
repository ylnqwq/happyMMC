...
SEED_BASE = env_int("MOIABC_ARCHIVE_RATE_SENSITIVITY_SEED_BASE", 20260723)
OUTPUT_DIR = env_output_dir(
    "MOIABC_ARCHIVE_RATE_SENSITIVITY_OUTPUT_DIR",
    MODULE_DIR / "moiabc_archive_rate_sensitivity_results",
    MODULE_DIR,
)
PARALLEL_WORKERS = env_int("MOIABC_ARCHIVE_RATE_SENSITIVITY_WORKERS", 4)
...
def run_once(benchmark, seed, archive_rate):
    ...
    archive_solutions, archive_objectives, _, used_seed = MOIABC.multi_objective_iabc(
        ...
        seed=seed,
        archive_guidance_rate=archive_rate,
        **COMMON_PARAMS,
    )
...
def run_once_task(task):
    benchmark, seed, archive_rate = task
    return run_once(benchmark, seed, archive_rate)


def print_configuration(benchmarks):
    total_runs = len(benchmarks) * len(ARCHIVE_RATES) * RUN_TIMES

    print("\n" + "=" * 80)
    print("MOIABC archive_rate sensitivity configuration")
...
def summarize_results(rows):
    grouped = {}
    for row in rows:
        key = (row["archive_rate"], row["benchmark_id"])
        grouped.setdefault(key, []).append(row)

    summary_rows = []
    for (archive_rate, benchmark_id), items in grouped.items():
...
def rank_parameter_sets(summary_rows):
    ...
    best_counts = {}
    metric_sums = {}
    for (_, metric_name), metric_rows in by_benchmark_metric.items():
        ...
        if ordered:
            best_key = ordered[0]["archive_rate"]
    ...
    rank_rows = []
    for archive_rate in ARCHIVE_RATES:
        ...
        rank_rows.append(
            {
                ...
                "best_count": best_counts.get(archive_rate, 0),
...
def main():
    ...
    total_tasks = len(benchmarks) * len(ARCHIVE_RATES) * RUN_TIMES
    rows = []
    done = 0
    start_time = time.perf_counter()

    tasks = [
        (benchmark, seed, archive_rate)
    ...
    if worker_count <= 1:
        for task in tasks:
            benchmark, _, archive_rate = task
            rows.append(run_once_task(task))
            done += 1
            prefix = f"{benchmark['id']} ar={archive_rate:.2f}"
        ...
        with ProcessPoolExecutor(max_workers=worker_count) as executor:
            ...
            for future in as_completed(futures):
                ...
                prefix = f"{row['benchmark_id']} ar={row['archive_rate']:.2f}"
                print_progress(done, total_tasks, prefix=prefix)
    ...
    save_rows_to_csv(OUTPUT_DIR / "moiabc_archive_rate_sensitivity_detail_results.csv", detail_rows)
    save_rows_to_csv(OUTPUT_DIR / "moiabc_archive_rate_sensitivity_summary_by_function.csv", summary_rows)
    save_rows_to_csv(OUTPUT_DIR / "moiabc_archive_rate_sensitivity_average_rank.csv", rank_rows)

    print("\n" + "=" * 80)
    print("MOIABC archive_rate sensitivity finished")
    ...
    if rank_rows:
        ...
        print(
            "Recommended params: "
            f"archive_rate={best['archive_rate']}, "
