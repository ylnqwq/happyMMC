...
SEED_BASE = env_int("MOIABC_SENSITIVITY_SEED_BASE", 20260723)
OUTPUT_DIR = env_output_dir(
    "MOIABC_ELITE_ELIMINATION_SENSITIVITY_OUTPUT_DIR",
    MODULE_DIR / "moiabc_elite_elimination_sensitivity_results",
    MODULE_DIR,
)
PARALLEL_WORKERS = env_int("MOIABC_SENSITIVITY_WORKERS", 8)
IGD_REFERENCE_POINTS = env_int("MOIABC_SENSITIVITY_IGD_REFERENCE_POINTS", 2000)
IGD_REFERENCE_CANDIDATES = env_int("MOIABC_SENSITIVITY_IGD_REFERENCE_CANDIDATES", 12000)
...
def attach_igd_metrics_to_rows(rows):
    ...
    for benchmark_id in benchmark_ids:
        ...
        for row in benchmark_rows:
            key = f"e={row['elite_rate']:.12g}|d={row['elimination_rate']:.12g}"
