...
def _load_shift(func_num, dimension):
    ...
    if key not in _CACHE:
        data = np.asarray(CEC2022_SHIFT[func_num], dtype=float)
        if func_num < 9:
...
def _load_rotation(func_num, dimension):
    ...
    key = ("rotation", func_num, dimension)
    if key not in _CACHE:
        data = np.asarray(CEC2022_ROTATION[func_num], dtype=float)
...
def _load_shuffle(func_num, dimension):
    ...
    if key not in _CACHE:
        data = CEC2022_SHUFFLE[func_num]
        _CACHE[key] = np.asarray(data, dtype=int).ravel()
    return _CACHE[key]
